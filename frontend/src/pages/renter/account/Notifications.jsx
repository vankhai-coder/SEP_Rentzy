...
const RenterNotifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get('/api/renter/notifications', { params: filters });
  ...
  const handleMarkAllAsRead = async () => {
    try {
      const response = await axiosInstance.patch('/api/renter/notifications/mark-all-read');
      if (response.data.success) {
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">Hiển thị và quản lý thông báo của bạn</p>
          </div>
          {unreadCount > 0 && (
            <button
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-gray-900">{(pagination.totalItems || 0) - unreadCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
        ...
        {notifications.length === 0 ? (
          <div className="px-6 py-12 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">Chưa có thông báo nào trong hệ thống.</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${typeColors[notification.type] || 'bg-gray-100 text-gray-800'}`}>
