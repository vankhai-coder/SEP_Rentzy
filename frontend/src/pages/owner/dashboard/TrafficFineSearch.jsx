...
const TrafficFineSearch = () => {
  ...
  const fetchCaptcha = async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get(`/api/owner/dashboard/traffic-fine-search/captcha`);
      ...
      setError('Không thể tải mã bảo mật. Vui lòng thử lại.');
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axiosInstance.post(`/api/owner/dashboard/traffic-fine-search`, {
  ...
  return (
    <div className="p-3 sm:p-4 lg:p-6 bg-secondary-100 dark:bg-secondary-950 min-h-screen overflow-x-hidden max-w-full">
      ...
      {/* Search Form */}
      <div className="bg-white dark:bg-secondary-800 rounded-lg shadow-sm border border-gray-200 dark:border-secondary-700 p-3 sm:p-4 md:p-6 mb-6 w-full max-w-full overflow-hidden">
              ...
              <div className="flex gap-2 items-center w-full min-w-0 max-w-full">
                <div className="flex gap-4 sm:gap-5 md:gap-6 items-center flex-shrink-0">
                  ...
                  <button
                    ...
                    className="p-1 sm:p-1.5 md:p-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {searchResults && (
        <div className="bg-white dark:bg-secondary-800 rounded-lg shadow-sm border border-gray-200 dark:border-secondary-700 p-6">
          <div className="flex items-center gap-3 mb-4">
            <MdReceipt className="text-2xl text-blue-600 dark:text-blue-400" />
          ...
          <div className="grid grid-cols-1 gap-4 mb-6">
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
              <p className="text-sm font-medium text-blue-700 dark:text-blue-300 mb-1">Tổng số vi phạm</p>
          ...
          {searchResults.violations && searchResults.violations.length > 0 ? (
            <div className="space-y-4">
              {searchResults.violations.map((violation, index) => (
                <div key={index} className="border border-gray-200 dark:border-secondary-700 rounded-lg p-4 hover:shadow-md dark:hover:bg-secondary-700 transition-shadow bg-white dark:bg-secondary-900">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                    <div className="flex items-start gap-2">
                      <MdAccessTime className="text-blue-600 dark:text-blue-400 mt-1 flex-shrink-0" />
                      <div>
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Thời gian vi phạm</p>
                        <p className="text-sm text-gray-900 dark:text-white">{violation.violationTime || 'N/A'}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      <MdLocationOn className="text-red-600 dark:text-red-400 mt-1 flex-shrink-0" />
                      <div>
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Địa điểm vi phạm</p>
                        <p className="text-sm text-gray-900 dark:text-white">{violation.violationLocation || 'N/A'}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      <MdBuild className="text-orange-600 dark:text-orange-400 mt-1 flex-shrink-0" />
                      <div>
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Hành vi vi phạm</p>
                        <p className="text-sm text-gray-900 dark:text-white">{violation.violationBehavior || 'N/A'}</p>
                      </div>
                    </div>
                    <div>
                      <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Đơn vị phát hiện</p>
                      <p className="text-sm text-gray-900 dark:text-white">{violation.detectionUnit || 'N/A'}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-200 dark:border-secondary-700">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Trạng thái</p>
                      ...
                      <div className="text-right">
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Loại phương tiện</p>
                        <p className="text-sm text-gray-900 dark:text-white">{violation.vehicleType || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {violation.resolutionPlaces && violation.resolutionPlaces.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-gray-200 dark:border-secondary-700">
                      <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Địa điểm giải quyết:</p>
                      <div className="space-y-2">
                        {violation.resolutionPlaces.map((place, placeIndex) => (
                          <div key={placeIndex} className="bg-gray-50 dark:bg-secondary-700 rounded p-2 text-sm">
                            <p className="font-medium text-gray-700 dark:text-gray-300">{place.name}</p>
                            {place.address && (
                              <p className="text-gray-600 dark:text-gray-400 text-xs mt-1">{place.address}</p>
                            )}
                          </div>
                        ))}
                      </div>
            ...
            <div className="text-center py-12 bg-gray-50 dark:bg-secondary-700 rounded-lg border border-gray-200 dark:border-secondary-600">
              <MdInfo className="mx-auto h-12 w-12 text-gray-400 dark:text-gray-500 mb-3" />
      ...
      {!searchResults && !loading && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 flex items-start gap-3">
          <MdInfo className="text-blue-600 dark:text-blue-400 text-xl flex-shrink-0 mt-0.5" />
